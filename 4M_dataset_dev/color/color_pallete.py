...
def process_image(image_path):
    text_file = image_path.replace("data", "color").replace(".jpg", ".txt")
    if os.path.exists(text_file):
        return  # Skip processing if the output file already exists
    ...
    k = 6
    kmeans = KMeans(pixels, k)
    colors = kmeans.get_colors()

    txt_path = os.path.dirname(text_file)
    os.makedirs(txt_path, exist_ok=True)

    with open(text_file, 'w') as f:
        f.write(str(colors))

images = glob.glob("/mnt/SSD1/Niantic/data/train/**/**/*.jpg") + glob.glob("/mnt/SSD1/Niantic/data/val/**/**/*.jpg")

if __name__ == '__main__':
    with Pool() as pool:
        for _ in tqdm(pool.imap_unordered(process_image, images), total=len(images), desc="Processing images"):
            pass
